...
def get_args():
    ...
    parser.add_argument('--alg', type=str, default='fedgroup',
                        help='communication strategy: fedavg/fedprox')
    parser.add_argument('--comm_round', type=int, default=50, help='number of maximum communication roun')
    parser.add_argument('--personal_round', type=int, default=10, help='number of personal roun')
...
def train_net(net_id, net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu", last_global_acc=1):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, target) in enumerate(train_dataloader):
            ...
            _,_,out = net(x)
            loss = criterion(out, target) + 10 * (1 - last_global_acc)
...
def local_train_net(nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model = None, prev_model_pool = None, server_c = None, clients_c = None, round=None, device="cpu", last_global_acc=None):
    avg_acc = 0.0
    acc_list = []
    if global_model:
        global_model.cuda()
    ...
    for net_id, net in nets.items():
        ...
        if args.alg == 'fedavg':
            trainacc, testacc = train_net(net_id, net, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args,
                                        device=device)
        elif args.alg == 'fedgroup':
            trainacc, testacc = train_net(net_id, net, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args,
                                        device=device, last_global_acc=last_global_acc)
...
def personal_local_train_net(nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model = None, prev_model_pool = None, server_c = None, clients_c = None, round=None, device="cpu", last_global_acc=None):
    avg_acc = 0.0
    acc_list = []
    if global_model:
        global_model.cuda()
...
def acc_local_train_net(nets, args, net_dataidx_map, train_dl=None, test_dl=None, global_model = None, prev_model_pool = None, server_c = None, clients_c = None, round=None, device="cpu", last_global_acc=None):
    avg_acc = 0.0
    acc_list = []
    if global_model:
        global_model.cuda()
...
if __name__ == '__main__':
    ...
    if args.server_momentum:
        ...
        for key in moment_v:
            moment_v[key] = 0
    if args.alg == 'moon':
        writer = SummaryWriter(log_dir='acc/moon/cifar100/b=0.3')
        old_nets_pool = []
        ...
        for round in range(n_comm_rounds):
            ...
            writer.add_scalar('testacc', test_acc, round)
            global_model.to('cpu')
        ...
        writer = SummaryWriter(log_dir='acc/fedavg/cifar100/b=0.3')
        for round in range(n_comm_rounds):
            logger.info("in comm round:" + str(round))
            party_list_this_round = party_list_rounds[round]
        ...
        writer = SummaryWriter(log_dir='acc/fedprox/cifar100/b=0.3')
        for round in range(n_comm_rounds):
            logger.info("in comm round:" + str(round))
            party_list_this_round = party_list_rounds[round]
            global_w = global_model.state_dict()
        ...
        writer = SummaryWriter(log_dir='acc/tinyimagenet/b=0.5/fedgroup/k=10')
        for round in range(n_comm_rounds):
            logger.info("in comm round:" + str(round))
            party_list_this_round = party_list_rounds[round]
            ...
            if round == 0:
                train_acc = 0
            this_nets, acc_list = acc_local_train_net(nets_this_round, args, net_dataidx_map, train_dl=train_dl, test_dl=test_dl, device=device, last_global_acc=train_acc)
            ...
            for net_id, net in enumerate(nets_this_round.values()):
                ...
                if net_id == 0:
                    for key in net_para:
                        # global_w[key] = net_para[key] * fed_avg_freqs[net_id]
                        global_w[key] = net_para[key] * acc_list[net_id]/total_acc
                else:
                    for key in net_para:
                        # global_w[key] += net_para[key] * fed_avg_freqs[net_id]
                        global_w[key] += net_para[key] * acc_list[net_id] / total_acc
            ...
            torch.save(global_model.state_dict(), args.modeldir+'fedgroup/'+'globalmodel'+args.log_file_name+'.pth')
            torch.save(nets[0].state_dict(), args.modeldir+'fedgroup/'+'localmodel0'+args.log_file_name+'.pth')

        # Personalize
        # nets_all = {k: nets[k] for k in party_list}
        # for net in nets_all.values():
        #     net.load_state_dict(global_w)
        # for rounds in range(args.personal_round):
        #     nets_all, test_acc = personal_local_train_net(nets_all, args, net_dataidx_map, train_dl=train_dl, test_dl=test_dl, device=device,
        #                             last_global_acc=train_acc)
        #     writer.add_scalar('testacc', test_acc, rounds + args.comm_round + 1)
